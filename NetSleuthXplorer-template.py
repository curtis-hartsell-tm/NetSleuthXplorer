...
# Function to get host reputation using MDTI API
def get_host_reputation(ip, access_token):
...
# Get unique DST_IP_ADDR for DNS resolution and reputation lookup
unique_dst_ip_addresses = significant_deviation['dst_ip_addr'].unique()

# Get access token
access_token = get_access_token(client_id, client_secret, tenant_id)

# Perform DNS resolution and IP reputation lookup with progress bar
dns_results = {}
reputation_results = {}
for ip in tqdm(unique_dst_ip_addresses, desc="Resolving DNS and Fetching IP Reputations", unit="IP"):
...
print(top_10_results[['src_ip_addr', 'dst_ip_addr', 'num_octets', 'z_score', 'dst_ip_dns', 'ip_reputation']])
